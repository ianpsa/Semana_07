...
Resposta:
```
varável idade = prompt('Insira sua idade')

se (idade for menor que 16) {
    print('Não pode votar!')
} se não, se (idade for menor ou igual 16 e idade menor que 18) { 
...
classe Retângulo extends FormaGeometrica:
    Atributos:
        -ladoMaior
        -ladoMenor

    Método Construtor(cor, ladoMaior e ladoMenor):
    super(cor)
    este.ladoMaior = ladoMaior
    este.ladoMenor = ladoMenor 

    Método CalcularArea(ladoMaior, ladoMenor):
    retornar ladoMaior*ladoMenor;

Classe Círculo extends FormaGeometrica:
    Atributos:
        -raio
    
    Método construtor(cor e raio):
    super(cor);
    este.raio = raio;
...
Resposta:
```
variável velocidadeMaxima = valor de velocidade máxima;
variável tempoMaximo = valor de tempo máximo;
variável velociadeInicial = valor de velocidade inicial;
variável distânciaFinal = valor da distanciaFinal;
variável aceleração = valor da aceleração;

se velocidade > velocidadeMaxima:
    retornar "velocidade maxima atingida";
    velocidade = velocidadeMaxima;

se tempo > tempoMaximo:
    retornar "passou do tempo máximo";
    velocidade = 0;

raiz(velocidadeInicial*(2*(distânciaFinal - distânciainicial)/(velocidadeFinal^2-velocidadeInicial^2)) = tempo
...
```
Resposta: só consegui pensar com matrizes quadraticas :(
```
Função Multiplicação de matrizes quadráticas:
se numero de linhas de MatrizA = numero de colunas de Matriz B então:
    retornar "as matrizes podem ser multiplicadas"
    colunas = tamanho(matrizA[1]); //pega o segundo numero da ordem da matriz
    linhas = tamanho(matrizB[0]); //pega o primeiro numero da ordem da matriz
    matrizResultante = novaMatriz(linhas, colunas)

    matrizResultante(1,1) = matrizA(1,1)*matrizB(1,1) + matrizA(1,2)*matrizB(2,1)
    matrizResultante(2,1) = matrizA(2,1)*matrizB(1,1) + matrizA(2,2)*matrizB(2,1)
    matrizResultante(1,2) = matrizA(1,1)*matrizB(1,2) + matrizA(1,2)*matrizB(2,2)
    matrizResultante(2,2) = matrizA(2,1)*matrizB(1,2) + matrizA(2,2)*matrizB(2,2)

    se não:
        retornar "as matrizes não podem ser multiplicadas"
    
retornar os valores  coluna 1: matrizResultante(1,1) matrizResultante(2,1) e valores coluna 2: matrizResultante(1,2) matrizResultante(2,2)
